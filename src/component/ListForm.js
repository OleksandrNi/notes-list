...
const ListForm = ({ setNotes, notes }) => {
  ...
  const deleteNote = (noteId) => {
    setNotes(notesMain.filter((note) => note.id !== noteId));
  };
        ...
        <div key={note.id} className="list">
          <div className="list__buttons">
            ...
            {note.isShowFuncButton && (
              <div className="list__button--main">
                <button
                  className="list__button--main-button"
          ...
          <div className="list__subnote">
            {note.notes.length > 0 && (
              <ListForm setNotes={setNotes} notes={note.notes} />
            )}
            {note.isShowAddSubButton && (
              <InputForm setNotes={setNotes} noteId={note.id} />
            )}
          </div>
        </div>
      ))}
